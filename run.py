...
def create_items(container):
    ...
    sales_order = get_sales_order("DirectionErgebnis")
...
def get_sales_order(item_id):
    order1 = {'id' : item_id,
            ...
            'purchase_order_number' : 'LONG',
            'order_date' : datetime.date(2023,1,10).strftime('%c'),
            'items' : [
                {'order_qty' : 1,
                    'product_id' : 100,
                }
...
def run_sample():
    ...
    try:
        ...
        read_item(container, 'DirectionErgebnis', 'Account1')
        read_items(container)
        query_items(container, 'Account1')
        #replace_item(container, 'SalesOrder1', 'Account1')
        upsert_item(container, 'DirectionErgebnis', 'Account1')
        delete_item(container, 'DirectionErgebnis', 'Account1')
